...
export const PatientHeader = ({ activeCase, patient, vitals, setActiveCase, handleUndo, history, showLabPanel, setShowLabPanel, isRunning, setIsRunning }) => {
  return (
    <header className="flex flex-col bg-slate-900 p-4 rounded-xl border border-slate-800 shadow-lg gap-4">
      ...
      <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center w-full gap-4">
        ...
        <div className="flex flex-wrap justify-end gap-2 w-full lg:w-auto">
          ...
          <button onClick={() => setShowLabPanel(!showLabPanel)} className="px-4 py-2 bg-blue-900/50 hover:bg-blue-800 text-blue-200 border border-blue-700 rounded text-xs md:text-sm font-bold flex items-center justify-center transition flex-1 lg:flex-none">EMR Labs</button>
          <button onClick={() => setIsRunning(!isRunning)} className={`px-6 py-2 rounded text-xs md:text-sm font-bold shadow-lg transition w-full lg:w-auto ${isRunning ? 'bg-red-600' : 'bg-green-600'}`}>{isRunning ? 'PAUSE SIM' : 'START SIM'}</button>
        </div>
      </div>
