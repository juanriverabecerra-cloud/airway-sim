  ...
  return (
    <div className="col-span-1 bg-slate-900 border border-slate-800 rounded-xl p-4 flex flex-col h-full max-h-[800px] shadow-2xl">
      <div className="flex justify-between items-center mb-3 border-b border-slate-700 pb-2">
        ...
        <button onClick={generateClinicalHint} className="bg-purple-900/40 hover:bg-purple-800/80 border border-purple-700 text-purple-300 px-3 py-1.5 rounded text-[10px] font-bold uppercase tracking-wider transition-all flex items-center gap-1 shadow-[0_0_10px_rgba(168,85,247,0.2)]">
          <Search size={12}/> Attending Consult
        </button>
      </div>
          ...
          if (log.includes('[STATUS]') && log.includes('[FORECAST]')) {
             ...
             return (
                <div key={index} className="flex flex-col gap-1.5 bg-purple-950/30 border border-purple-900/50 rounded-lg p-2.5 my-2">
                   ...
                   <div className="text-[11px] text-slate-200"><span className="font-black text-white bg-slate-800 px-1 rounded mr-1">STATUS</span> {statusStr}</div>
                   <div className="text-[11px] text-purple-200"><span className="font-black text-white bg-purple-900 px-1 rounded mr-1">FORECAST</span> {forecastStr}</div>
                </div>
             );
          }
