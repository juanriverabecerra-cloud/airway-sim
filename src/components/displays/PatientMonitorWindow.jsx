...
import { PrimaryMonitor } from '../monitors/PrimaryMonitor';
import { VitalContextPanel } from '../VitalContextPanel';
import { WaveformContextPanel } from '../WaveformContextPanel';
import { EkgModal } from '../modals/EkgModal';
import { Activity, Maximize2, ShieldAlert } from 'lucide-react';

export function PatientMonitorWindow() {
  const { state: syncedState, actions } = useSimState();
  const [vitalContext, setVitalContext] = useState(null);
  ...
  // Mask vitals if blindMode is active
  const maskedVitals = blindMode && vitals ? { ...vitals, spo2: '??', etco2: '??' } : vitals;
  ...
  return (
    <div className="w-screen h-screen bg-slate-950 text-slate-100 flex flex-col overflow-hidden select-none font-sans relative">
      ...
      <div className="h-10 bg-slate-900 border-b border-slate-800 px-4 flex items-center justify-between shrink-0">
        <div className="flex items-center gap-3">
          ...
          <span className="text-xs font-black tracking-wider uppercase text-slate-200">
            AETHERIS MULTIPARAMETER PATIENT MONITOR
          </span>
        ...
        {blindMode && (
          <div className="flex items-center gap-1.5 px-2.5 py-1 rounded bg-amber-500/20 border border-amber-500/40 text-amber-300 text-xs font-bold">
            ...
            BLIND EXAM MODE (SPO2/ETCO2 MASKED)
          </div>
        )}
        ...
        {!patient || !vitals ? (
          ...
          <div className="w-full h-full rounded-xl border border-slate-800 bg-slate-900/60 overflow-hidden relative">
            <PrimaryMonitor
              ...
              patient={patient}
              nibp={nibp}
              cycleNibp={actions.cycleNibp}
              isCyclingNibp={syncedState.isCyclingNibp}
              nibpIntervalMs={syncedState.nibpIntervalMs}
              setNibpIntervalMs={actions.setNibpIntervalMs}
              gasSettings={syncedState.gasSettings}
              ventSettings={ventSettings}
              electrolytes={syncedState.electrolytes}
              activeMeds={syncedState.activeMeds}
              soundSettings={syncedState.soundSettings}
      ...
      {/* Vital Context Panel Overlay */}
      {vitalContext && vitals && patient && (
        <VitalContextPanel
          vitalId={vitalContext.id}
          vitals={vitals}
          patient={patient}
          onClose={() => setVitalContext(null)}
          onOpenEkg={() => setEkgModalOpen(true)}
        />
      )}

      {/* Waveform Context Panel Overlay */}
