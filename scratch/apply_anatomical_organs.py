...
new_body_svg = """const BodySvg = ({ activity, hoveredSite, onSiteEnter, onSiteLeave }) => {
  ...
  return (
    <svg viewBox="0 0 360 510" className="w-full select-none" style={{ height: "auto", display: "block" }}>
      ...
      <ellipse cx="206" cy="208" rx="16" ry="18" {...region('cardiac')}/>
      <ellipse cx="154" cy="205" rx="14" ry="22" {...region('pulmonary')}/>
      <ellipse cx="206" cy="205" rx="14" ry="22" {...region('pulmonary')}/>
      ...
      <path d="M 206,198 L 206,175 C 206,165 186,164 186,175 L 186,310" {...line('arterial', 2.0)}/>
      {/* SVC */}
      ...
      {/* Heart: anatomical tilted ventricles + aorta loop */}
      <path d="M 200,192 C 194,192 192,200 192,208 C 192,218 206,225 208,225 C 210,225 218,216 218,206 C 218,198 212,192 205,192 L 205,183 C 205,178 198,178 198,183 L 200,192 Z" {...vis('cardiac')}/>

      {/* Lungs: bilateral lobes with cardiac notch and fissures */}
      {/* Viewer's Left Lung (Patient's Right Lung - 3 lobes) */}
      <path d="M 155,185 C 144,187 138,202 138,222 C 138,234 146,236 155,227 Z" {...vis('pulmonary')}/>
      <line x1="139" y1="205" x2="153" y2="225" stroke={sg.pulmonary.color} strokeOpacity={sg.pulmonary.occ > 0.05 ? 0.6 : 0.3} strokeWidth="0.8" style={{pointerEvents:'none'}}/>
      <line x1="145" y1="213" x2="155" y2="213" stroke={sg.pulmonary.color} strokeOpacity={sg.pulmonary.occ > 0.05 ? 0.6 : 0.3} strokeWidth="0.8" style={{pointerEvents:'none'}}/>

      {/* Viewer's Right Lung (Patient's Left Lung - 2 lobes + cardiac notch) */}
      <path d="M 205,185 C 216,187 222,202 222,222 C 222,234 214,236 205,227 C 209,222 212,216 211,210 C 210,204 205,200 205,185 Z" {...vis('pulmonary')}/>
      <line x1="221" y1="205" x2="207" y2="225" stroke={sg.pulmonary.color} strokeOpacity={sg.pulmonary.occ > 0.05 ? 0.6 : 0.3} strokeWidth="0.8" style={{pointerEvents:'none'}}/>
      ...
      {/* GI: J-shaped Stomach + Large Colon frame + small intestine loops */}
      {/* Stomach */}
      <path d="M 174,242 C 164,242 158,250 158,260 C 158,272 172,274 184,270 C 188,268 190,262 186,258 C 180,254 175,254 172,250 C 170,247 172,244 174,242 Z" {...vis('gi')}/>
      {/* Haustrated Large Colon */}
      <path d="M 164,300 Q 161,295 164,290 Q 161,285 164,280 Q 162,275 166,275 Q 171,278 176,275 Q 181,278 186,275 Q 191,278 194,275 Q 198,275 196,280 Q 199,285 196,290 Q 199,295 196,300 Q 188,304 180,302 Q 172,304 164,300 Z" {...vis('gi')}/>
      {/* Small Intestines loops */}
      <path d="M 170,282 C 168,288 174,292 180,288 C 186,292 192,288 190,282 C 188,276 172,276 170,282 C 168,288 180,294 180,286 C 180,294 192,288 190,282" fill="none" {...line('gi', 0.9)}/>
      ...
      {/* Lungs */}
      <ellipse cx="154" cy="205" rx="20" ry="30" {...hit()} onMouseEnter={()=>onSiteEnter('pulmonary')} onMouseLeave={onSiteLeave}/>
      <ellipse cx="206" cy="205" rx="20" ry="30" {...hit()} onMouseEnter={()=>onSiteEnter('pulmonary')} onMouseLeave={onSiteLeave}/>
      {/* Heart */}
      <ellipse cx="206" cy="208" rx="24" ry="22" {...hit()} onMouseEnter={()=>onSiteEnter('cardiac')} onMouseLeave={onSiteLeave}/>
      {/* Arterial */}
...
print("ANATOMICAL ORGANS ASSEMBLY COMPLETE!")
